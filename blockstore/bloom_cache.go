...
// Cleanup clears the bloom filter (for memory management)
func (b *bloomcache) Cleanup(ratio float64) {
	if ratio <= 0 || ratio > 1 {
		return
	}
	
	// For bloom filters, we can't partially clean, so we reset if ratio is significant
	if ratio >= 0.3 { // Reset bloom filter if 30% or more cleanup requested
		atomic.StoreInt32(&b.active, 0)
		
		// Create new bloom filter with same parameters
		if bl, err := bloom.New(float64(b.bloom.ElementsAdded()), 7); err == nil { // Use default 7 hash functions
			b.bloom = bl
			logger.Debug("Reset bloom filter due to memory pressure")
			
			// Rebuild in background
			go func() {
				if err := b.build(context.Background()); err != nil {
					logger.Errorf("Failed to rebuild bloom filter after cleanup: %v", err)
				}
			}()
		}
	}
