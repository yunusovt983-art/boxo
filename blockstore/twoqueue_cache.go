...
// Cleanup removes a percentage of cache entries (for memory management)
func (b *tqcache) Cleanup(ratio float64) {
	if ratio <= 0 || ratio > 1 {
		return
	}
	
	// Get current cache size
	currentSize := b.cache.Len()
	if currentSize == 0 {
		return
	}
	
	// Calculate how many entries to remove
	toRemove := int(float64(currentSize) * ratio)
	if toRemove == 0 {
		return
	}
	
	// Remove entries from cache
	// Note: TwoQueueCache doesn't have a direct way to remove multiple entries,
	// so we'll purge the entire cache if ratio is high enough
	if ratio >= 0.5 {
		b.cache.Purge()
		logger.Debugf("Purged entire TwoQueue cache due to memory pressure")
	} else {
		// For smaller ratios, we can't selectively remove entries from golang-lru,
		// so we'll just log the request
		logger.Debugf("TwoQueue cache cleanup requested (ratio: %.2f) but selective removal not supported", ratio)
	}
}
